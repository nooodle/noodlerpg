...
exports.refuel = function(req, db, callback) {
  ...
  var refuelAmount = parseInt(req.body.amount, 10);
  var refuelCost = parseInt(req.body.cost, 10);

  if (gold >= refuelCost) {
    req.session.hp = parseInt(req.session.hp, 10) + refuelAmount;
    req.session.gold = gold - refuelCost;
